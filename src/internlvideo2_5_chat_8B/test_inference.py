#!/usr/bin/env python3
"""
Test script for InternVideo2.5_Chat_8B Traffic Inference Pipeline.

Tests the InternVideo2.5_Chat_8B inference on:
- Multiple video samples
- Memory monitoring
- Accuracy metrics (when ground truth available)
- Duration-based frame calculation
...
Usage:
    ...
    python test_inference.py --use-duration-frames  # HuggingFace example
"""
...
def test_inference():
    ...
    parser = argparse.ArgumentParser(
        description="Test InternVideo2.5_Chat_8B traffic inference on sample videos"
    )
    parser.add_argument(
        "--model",
        type=str,
    ...
    print("\n" + "=" * 70)
    print("INTERNVIDEO2.5_CHAT_8B TRAFFIC INFERENCE TEST")
    print("=" * 70)
    ...
    try:
        ...
        if "out of memory" in str(e).lower():
            ...
            print(f"   3. Reduce max_num: --max-num 8")
            print(f"   4. Reduce frame count: --min-frames 8 --max-frames 24")
            print(f"   5. Disable duration frames: remove --use-duration-frames")
    ...
    print(f"  python road_buddy/src/internlvideo2_5_chat_8B/run_inference.py")
    print()
