"""
InternVL3.5-4B Video Inference Pipeline.

Enhanced inference for dashcam videos using InternVL3.5-4B with:
- Flash Attention for memory efficiency (~30% memory savings)
- Multi-frame processing (supports up to 512 frames)
- Optimized for 12GB+ VRAM (smaller model than 8B)
- Quantization support (8-bit/4-bit) for lower-end GPUs
"""
...
class InternVL35_4BInference:
    """
    InternVL3.5-4B inference pipeline for dashcam videos.

    This implementation uses Flash Attention for memory efficiency and is
    optimized for 12GB+ VRAM. The 4B model is more memory efficient than 8B.
    """
    ...
    def __init__(
        ...
        Args:
            model_name: HuggingFace model name (default: OpenGVLab/InternVL3_5-4B)
        ...
        print(f"\n{'='*60}")
        print(f"InternVL3.5-4B Inference Pipeline")
        print(f"{'='*60}")
        ...
        if device == "cpu":
            ...
            print("   - Requires significant RAM (~4-8 GB for this model)")
        ...
        if device == "cuda" and torch.cuda.is_available():
            ...
            if free_mem < 8:
                ...
                print("   - InternVL3.5-4B with flash attention requires 8-12 GB")
                print("   - Consider using --load-in-8bit for safety margin\n")

        # Load model
        print("Loading InternVL3.5-4B model...")
        try:
            ...
            if device == "cuda" and torch.cuda.is_available():
                ...
                if load_in_8bit and allocated > 8:
                    print(f"  ⚠️  Warning: 8-bit model using {allocated:.2f} GB (expected ~4-6 GB)")
                    print(f"     Quantization may not have worked properly")
                elif load_in_4bit and allocated > 6:
                    print(f"  ⚠️  Warning: 4-bit model using {allocated:.2f} GB (expected ~2-4 GB)")
                    print(f"     Quantization may not have worked properly")
                elif load_in_8bit:
                    print(f"  ✓ 8-bit quantization successful (~{100 - (allocated/10)*100:.0f}% memory reduction)")
                elif load_in_4bit:
                    print(f"  ✓ 4-bit quantization successful (~{100 - (allocated/10)*100:.0f}% memory reduction)")
    ...
    def infer(
        ...
        """
        Run inference on a single video using InternVL3.5-4B.
...
def main():
    ...
    pipeline = InternVL35_4BInference(
        ...
        min_frames=2,  # Extreme reduction to avoid OOM
        max_frames=4,  # Extreme reduction to avoid OOM
        max_num=1,     # No tiling to minimize memory usage
