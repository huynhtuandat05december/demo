#!/usr/bin/env python3
"""
Test script for InternVL3.5-4B Traffic Inference Pipeline.

Tests the InternVL3.5-4B inference with Flash Attention on:
- Multiple video samples
- Memory monitoring (optimized for 12GB+ VRAM)
...
def test_inference():
    ...
    parser = argparse.ArgumentParser(
        description="Test InternVL3.5-4B traffic inference on sample videos"
    )
    parser.add_argument(
        "--model",
        type=str,
    ...
    print("\n" + "=" * 70)
    print("INTERNVL3.5-4B TRAFFIC INFERENCE TEST")
    print("=" * 70)
    ...
    if args.device == "cuda" and torch.cuda.is_available():
        ...
        # 12GB+ VRAM check
        if peak_memory <= 12:
            print(f"✅ Fits comfortably in 12GB VRAM (peak: {peak_memory:.2f} GB)")
        elif peak_memory <= 16:
            print(f"✅ Fits in 16GB VRAM (peak: {peak_memory:.2f} GB)")
        else:
            print(f"⚠️  High memory usage (peak: {peak_memory:.2f} GB)")
    ...
    print(f"  python road_buddy/src/internvl3_5_4B/run_inference.py")
    print()
